...
export default function WhyUsSection() {
    return (
        <section className="py-24 relative bg-navy border-t border-border" id="why-us">
            {/* Background ambient glow */}
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(79,142,247,0.05)_0%,transparent_70%)] pointer-events-none" />
            ...
            <div className="container-custom mx-auto relative z-10">
                ...
                <div className="flex flex-col md:flex-row items-end justify-between gap-8 mb-16 reveal">
                    <div className="max-w-2xl">
                        ...
                        <h2 className="font-display font-black text-4xl md:text-5xl lg:text-6xl text-text-1 tracking-tight leading-tight">
                            Features That <br />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 reveal-up delay-200">
                    {cards.map((card, i) => (
                        <motion.div
                            ...
                            key={card.id}
                            className="group bg-navy-mid border border-border hover:border-primary/20 hover:bg-navy hover:shadow-2xl transition-all duration-500 rounded-[2.5rem] p-8 md:p-12 flex flex-col justify-between overflow-hidden relative"
                        >
                            ...
                            <div className="flex justify-between items-start mb-12 relative z-10">
                                <div
                                    className="w-16 h-16 rounded-2xl flex items-center justify-center border transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3 shadow-sm bg-navy"
                                    style={{ borderColor: `${card.color}20` }}
                                >
                                    <card.icon className="w-8 h-8" style={{ color: card.color }} />
                                </div>
                                <span className="text-white/5 font-display font-black text-6xl leading-none group-hover:text-primary/10 transition-colors">
                                    0{card.id}
                                </span>
                            </div>

                            <div className="relative z-10">
                                <h3 className="font-display font-black text-2xl text-text-1 mb-4 group-hover:text-primary transition-all">
                                    {card.title}
                                </h3>
                                <p className="text-text-3 text-base md:text-lg leading-relaxed font-medium">
                                    {card.desc}
                                </p>
                            </div>
                        </motion.div>
