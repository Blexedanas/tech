...
import { ArrowRight, ArrowLeft, Check, Code, Smartphone, Brain, Palette, Cpu, Globe, ShoppingCart, ShieldCheck } from 'lucide-react'
...
export default function ProjectEstimator() {
    ...
    return (
        <div className="bg-navy-card border border-white/5 rounded-3xl p-8 lg:p-12 shadow-2xl relative overflow-hidden text-white w-full max-w-4xl mx-auto">
            {/* Background Glow */}
            ...
            <div className="mb-10 relative z-10">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-white/60">Get an instant, customized cost estimation.</p>
                    </div>
                ...
                <div className="flex gap-2">
                    {[1, 2, 3, 4].map(i => (
                        <div key={i} className={`h-1.5 rounded-full flex-1 transition-all duration-500 ${step >= i ? 'bg-primary' : 'bg-white/10'}`} />
                    ))}
                </div>
            </div>
            ...
            <div className="relative min-h-[400px] z-10">
                <AnimatePresence mode='wait'>
                    ...
                    {step === 1 && (
                        ...
                        >
                            <h4 className="text-xl font-bold mb-6">1. What services do you need? <span className="text-white/40 font-normal text-sm ml-2">(Select all that apply)</span></h4>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                {SERVICE_OPTIONS.map(service => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={`p-2 rounded-lg ${isSelected ? 'bg-primary text-white' : 'bg-white/10 text-white/60'}`}>
                                ...
                                {COMPLEXITY_OPTIONS.map(comp => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${isSelected ? 'border-primary' : 'border-white/20'}`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {TIMELINE_OPTIONS.map(time => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={`w-12 h-12 rounded-full border flex items-center justify-center ${isSelected ? 'border-primary bg-primary text-white' : 'border-white/20 text-white/40'}`}>
                    ...
                    {step === 4 && (
                        ...
                        >
                            ...
                            <p className="text-white/60 mb-8 max-w-md mx-auto">This is a rough estimate based on your selections. Let&apos;s talk specifics to refine it.</p>
                            ...
                            <div className="bg-white/5 border border-white/10 rounded-3xl p-8 mb-8 inline-block min-w-[300px]">
                                ...
                                <div className="font-mono text-5xl font-black text-white">
                                    ₹{(estimatedPrice * 0.9).toLocaleString()}<span className="text-white/40 font-sans font-normal text-3xl mx-2">-</span>₹{(estimatedPrice * 1.1).toLocaleString()}
                                </div>
                            </div>
            ...
            {step < 4 && (
                <div className="mt-10 flex justify-between items-center border-t border-white/10 pt-6 relative z-10">
                    <button
                        onClick={prevStep}
                        disabled={step === 1}
                        className={`flex items-center gap-2 px-4 py-2 font-bold transition-all ${step === 1 ? 'opacity-0 pointer-events-none' : 'text-white/60 hover:text-white'}`}
                    >
